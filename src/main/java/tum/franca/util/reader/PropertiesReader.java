package tum.franca.util.reader;
...
import tum.franca.util.characteristics.CharacteristicsWrapper;
import tum.franca.util.characteristics.CharacteristicsWrapper.Properties;
...
public class PropertiesReader extends InterfaceReader {
	...
	public Properties.FUNCTIONALSCOPE getFunctionalScopeProperties() {
		return CharacteristicsWrapper.functionaScopeHashMap.get(getFirstInterface().getFunctionalScope().getName());
	}
	...
	public Properties.SAFETYCRITICAL getSafetyCriticalProperties() {
		return CharacteristicsWrapper.safetyCriticalHashMap.get(getFirstInterface().getSafetyCritical().getName());
	}
	...
	public Properties.SECURITYCRITICAL getSecurityCriticalProperties() {
		return CharacteristicsWrapper.securityCriticalHashMap.get(getFirstInterface().getSecurityCritical().getName());
	}
	...
	public Properties.TIMESPECIFICATION getTimeSpecifiactionProperties() {
		return CharacteristicsWrapper.timeSpecificationHashMap.get(getFirstInterface().getTimeSpecification().getName());
	}
	...
	public Properties.RUNTIME getRuntimeProperties() {
		return CharacteristicsWrapper.runtimeHashMap.get(getFirstInterface().getRuntime().getName());
	}
